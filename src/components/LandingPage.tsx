...
import { GraduationCap, Sparkles, BookOpen, PenLine, BarChart3, Users, ArrowRight, ChevronDown, Brain, Shield, Zap } from 'lucide-react';

const REGION_COLORS = ['#3b82f6', '#22c55e', '#eab308', '#ef4444', '#a855f7'];
...
function DoughnutChart({ data }: { data?: Record<string, number> }) {
  ...
  const slices = values.map((v, i) => {
    ...
    return { percent, startPercent, color: REGION_COLORS[i], label: REGIONS[i], value: v };
  });
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="relative flex items-center justify-center">
        ...
        <div className="absolute flex flex-col items-center pointer-events-none">
          <span className="text-3xl font-extrabold text-slate-800 dark:text-white">{totalVal}</span>
...
export default function LandingPage({ onStart, onSignup }: { onStart: () => void; onSignup?: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0814] text-slate-800 dark:text-slate-100">
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-[#0a0814]/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-500 dark:text-slate-400">
            ...
            <button type="button" onClick={() => scrollTo('cta')} className="hover:text-blue-600 dark:hover:text-blue-400 transition cursor-pointer">Começar</button>
          </nav>
          <div className="flex items-center gap-3">
            <button type="button" onClick={onStart} className="px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-blue-600 transition cursor-pointer">
              Entrar
            </button>
            <button type="button" onClick={onSignup} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition cursor-pointer">
              Cadastrar
            </button>
          </div>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-3xl mx-auto">
            {[
              ...
              <div key={i} className="p-4 rounded-2xl bg-slate-50 dark:bg-[#1e293b]/50 border border-slate-100 dark:border-slate-800 text-center">
      ...
      <section id="features" className="py-20 md:py-28 bg-slate-50/50 dark:bg-[#0f0a1e]/50">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Zap, title: 'Correção de Redação', desc: 'Envie sua redação e receba correção detalhada seguindo as 5 competências do ENEM, com nota de 0 a 1000, feedback por competência e sugestões de melhoria.', color: 'from-blue-500 to-blue-600' },
              { icon: Brain, title: 'Questões Mágicas', desc: 'A IA gera questões inéditas e personalizadas no estilo ENEM, focadas exatamente nas áreas onde você precisa treinar mais.', color: 'from-purple-500 to-purple-600' },
              { icon: BarChart3, title: 'Simulados Reais', desc: 'Resolva questões autênticas do ENEM dos anos anteriores, com correção automática e análise detalhada do seu desempenho.', color: 'from-emerald-500 to-emerald-600' },
              ...
              { icon: Users, title: 'Acompanhamento', desc: 'Veja sua evolução ao longo do tempo com gráficos e estatísticas — perfeito para monitorar seu progresso.', color: 'from-rose-500 to-rose-600' },
              ...
              <div key={i} className="p-6 rounded-2xl bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-800 hover:border-blue-300 dark:hover:border-blue-700 transition group">
      ...
      <section id="stats-section" className="py-20 md:py-28">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
      ...
      <section id="cta" className="py-20 md:py-28 bg-gradient-to-br from-blue-600 to-purple-700 text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button type="button" onClick={onSignup} className="px-10 py-4 bg-white text-blue-700 font-bold text-sm rounded-xl transition shadow-xl hover:shadow-2xl hover:scale-105 flex items-center gap-2 cursor-pointer">
              Criar Conta Grátis <ArrowRight className="h-4 w-4" />
            </button>
