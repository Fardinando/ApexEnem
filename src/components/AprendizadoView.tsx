...
export default function AprendizadoView({
  ...
  const [chapterQuizChecked, setChapterQuizChecked] = useState<Record<number, number | null>>({});
  const [chapterQuizRevealed, setChapterQuizRevealed] = useState<Record<number, boolean>>({});
  ...
  const handleBackToCategories = () => {
    ...
    setChapterQuizChecked({});
    setChapterQuizRevealed({});
  };
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
            ...
            {isInteractive && section.options && (() => {
              const chRevealed = chapterQuizRevealed[chapterStep] || false;
              const chSelected = chapterQuizChecked[chapterStep];
              return (
              <div className="space-y-3 pt-2">
                {section.options.map((opt, i) => {
                  const isSelected = chSelected === i;
                  let optClass = 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50';
                  if (chRevealed) {
                  ...
                  return (
                    <button key={i} type="button" disabled={chRevealed}
                      onClick={() => !chRevealed && setChapterQuizChecked(prev => ({ ...prev, [chapterStep]: i }))}
                      className={`w-full text-left p-3 rounded-xl border text-xs text-slate-700 dark:text-slate-300 transition cursor-pointer ${optClass}`}>
                      <span className="font-bold mr-2">{String.fromCharCode(65 + i)})</span>{opt}
                    </button>
                  );
                })}
                {!chRevealed ? (
                  <button type="button" disabled={chSelected === null && chSelected === undefined}
                    onClick={() => setChapterQuizRevealed(prev => ({ ...prev, [chapterStep]: true }))}
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white rounded-xl text-xs font-bold transition cursor-pointer">
                    Verificar Resposta
                  </button>
                ) : (
                  section.explanation && (
            ...
            {isQuiz && section.questions && (
              <div className="space-y-4 pt-2">
                {section.questions.map((q, qi) => {
                  const quizKey = chapterStep * 100 + qi;
                  const quizRevealed = chapterQuizRevealed[quizKey] || false;
                  const quizSelected = chapterQuizChecked[quizKey];
                  return (
                  <div key={qi} className="p-4 bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
                    ...
                    {q.options.map((opt, oi) => {
                      ...
                      const isOptSelected = quizSelected === oi;
                      let optCls = 'border-slate-200 dark:border-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800/50';
                      if (quizRevealed) {
                        optCls = isOptCorrect
                          ? 'bg-green-50 dark:bg-green-950/20 border-green-300 dark:border-green-700 text-green-700 dark:text-green-400 font-bold'
                      ...
                      return (
                        <button key={oi} type="button" disabled={quizRevealed}
                          onClick={() => !quizRevealed && setChapterQuizChecked(prev => ({ ...prev, [quizKey]: oi }))}
                          className={`w-full text-left text-[11px] px-3 py-1.5 rounded-lg border cursor-pointer transition ${optCls} ${isOptSelected && !quizRevealed ? 'ring-2 ring-blue-400' : ''}`}>
                          {String.fromCharCode(65 + oi)}) {opt}
                        </button>
                      );
                    })}
                    {!quizRevealed ? (
                      <button type="button" disabled={quizSelected === undefined}
                        onClick={() => setChapterQuizRevealed(prev => ({ ...prev, [quizKey]: true }))}
                        className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white rounded-lg text-[10px] font-bold transition cursor-pointer">
                        Verificar
                      </button>
                    ) : (
  ...
  return (
    <>
      <LoadingOverlay isVisible={loadingChapterLesson} title="Preparando sua Aula" subtitle="O Cabrito do Mil está montando a lição personalizada pra você..." />
      <div
        id="aprendizado-view"
