  ...
  const doCorrection = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const err = new Error(errData?.error || `Erro do servidor (${res.status})`);
        (err as any).details = errData?.details?.join?.('\n') || null;
        throw err;
      }
  ...
  const handleReportError = async () => {
    ...
    try {
      ...
      await fetch(`${APEXGUARDIAN_URL}/webhook/report`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          description: `Erro na correção de redação: ${errorMessage}${errorDetails ? `\nDetalhes: ${errorDetails}` : ''}`,
          timestamp_frontend: Math.floor(Date.now() / 1000),
          user_id_anon: hashHex,
        }),
      });
      alert('Erro reportado com sucesso! O ApexGuardian vai investigar.');
      setErrorMessage(null);
      setErrorDetails(null);
    } catch {
      alert('Falha ao reportar erro. Tente novamente.');
    } finally {
      setReporting(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start" id="split-view-columns">
        ...
        <div className="lg:col-span-6" id="results-column">
          ...
          {!isLoading && errorMessage && (
            <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-red-200 dark:border-red-900/40 p-8 text-center space-y-4 shadow-sm min-h-[440px] flex flex-col justify-center items-center">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-4 py-2 text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-[#0f172a] transition cursor-pointer"
                >
                  Fechar
                </button>
                <button
                  ...
                  disabled={reporting}
                  className="px-4 py-2 text-xs font-semibold text-white bg-red-600 hover:bg-red-700 rounded-xl transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                >
                  <Bug className="h-3.5 w-3.5" />
                  {reporting ? 'Reportando...' : 'Reportar Erro'}
                </button>
              </div>
            </div>
          )}
